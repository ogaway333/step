...
<form class="p-form c-container-form" method="POST" action="{{route('mystep.edit', ['step_id' => $step->id])}}">
    ...
    @enderror
    <textarea name="content" id="content" class="p-form__textarea @error('content') is-invalid @enderror" required autocomplete="name" cols="30" rows="10">{{ $step->content }}</textarea>
